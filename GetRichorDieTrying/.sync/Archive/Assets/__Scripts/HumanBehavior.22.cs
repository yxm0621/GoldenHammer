...
//			iTween.MoveTo (human, iTween.Hash ("z", downPos.transform.position.z, "time", movementSpeed, "easetype", iTween.EaseType.linear));
	...
	public void Move(){
		...
		//Move using itween
			...
			if(startedLeft){
			Debug.Log ("Moving down road");
				iTween.MoveTo(human, iTween.Hash ("z", leftDownPos, "time", 0.5f, "easetype", iTween.EaseType.linear));

			}
			
		if(startedRight){
			Debug.Log ("Moving down road");
			iTween.MoveTo(human, iTween.Hash ("z", rightDownPos, "time", 0.5f, "easetype", iTween.EaseType.linear));
		}
