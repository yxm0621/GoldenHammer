...
namespace Google2u
{
    ...
    public class Google2uAccountWorkbook : WorkbookBase
    {
            ...
            if (WorksheetQueryStatus == QueryStatus.QueryComplete)
            {
                foreach (var Google2uWorksheet in Worksheets)
                {
                    if (ActiveWorksheetIndex > -1 && Worksheets[ActiveWorksheetIndex] != null && Google2uWorksheet == Worksheets[ActiveWorksheetIndex])
                        Google2uWorksheet.Update(true);
                    else Google2uWorksheet.Update(false);
                }
            }
        ...
        public override bool DrawGUIList(EditorGUILayoutEx in_layout)
        {
            var ret = true;
            ShowSpreadsheet = Google2uGUIUtil.GetBool("workbook" + WorkbookName.Replace(' ', '_') + "_Open", ShowSpreadsheet);
            var mainFadeArea = in_layout.BeginFadeArea(ShowSpreadsheet, WorkbookName, "workbook" + WorkbookName.Replace(' ', '_'), in_layout.OuterBox, in_layout.OuterBoxHeader);
