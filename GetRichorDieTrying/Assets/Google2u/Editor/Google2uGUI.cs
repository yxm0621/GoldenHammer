...
namespace Google2u
{
    public partial class Google2u : EditorWindow
    {
        ...
        public void LoadPathSettings()
        {
            ...
            _StaticDbResourcesDirectory = Google2uGUIUtil.GetString("g2uStaticDBResourcesDirectory", _StaticDbResourcesDirectory);
        ...
        public void LoadLanguageSettings()
        {
            ...
            LocalizationInfo.EditorLanguage = Google2uGUIUtil.GetString("editorLanguage", LocalizationInfo.EditorLanguage);
        ...
        public void LoadManualWorkbooks()
        {
            ...
            var manualWorkbookCache = Google2uGUIUtil.GetString(InstanceData.ProjectPath + "_ManualWorkbookCache", string.Empty);
        ...
        public void LoadVisibilitySettings()
        {
            ...
            ShowSettingsPathGameObjectExport = Google2uGUIUtil.GetBool("ShowSettingsPathGameObjectExport", ShowSettingsPathGameObjectExport);
            ShowSettingsPathStaticExport = Google2uGUIUtil.GetBool("ShowSettingsPathStaticExport", ShowSettingsPathStaticExport);
            ShowSettingsPathXMLExport = Google2uGUIUtil.GetBool("ShowSettingsPathXMLExport", ShowSettingsPathXMLExport);
            ShowSettingsPathJSONExport = Google2uGUIUtil.GetBool("ShowSettingsPathJSONExport", ShowSettingsPathJSONExport);
            ShowSettingsPathCSVExport = Google2uGUIUtil.GetBool("ShowSettingsPathCSVExport", ShowSettingsPathCSVExport);
            ShowSettingsPathNGUIExport = Google2uGUIUtil.GetBool("ShowSettingsPathNGUIExport", ShowSettingsPathNGUIExport);
            ShowSettingsPathPlaymakerExport = Google2uGUIUtil.GetBool("ShowSettingsPathPlaymakerExport", ShowSettingsPathPlaymakerExport);
        ...
        void DrawLoginGUI()
        {
            if (InstanceData.Service == null)
                Init();
            var loginFadeArea = string.IsNullOrEmpty(Token)
                ...
                : MyGUILayout.BeginFadeArea(ShowLogin,
                    LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_LOGGED_IN_AS) + ": " +
                    Username, "login", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
            ...
            if (loginFadeArea.Show())
            {
                ...
                else
                {
                    ...
                    {
                        Username =
                            EditorGUILayoutEx.StringInput(
                                LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_USERNAME), Username,
                                "SavedUsername", _SaveCredentials);
                        Password =
                            EditorGUILayoutEx.PasswordInput(
                                LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_PASSWORD),
                                Password, "SavedPassword", _SaveCredentials);

                        var oldSaveCredentials = _SaveCredentials;
                        EditorGUILayoutEx.ToggleInput(
                            LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_SAVE_CREDENTIALS),
                            ref _SaveCredentials, "SaveCredentials");

                        if (_SaveCredentials != oldSaveCredentials)
                        {
                            if (_SaveCredentials)
                            {
                                Google2uGUIUtil.SetString("SavedUsername", Username);
                                Google2uGUIUtil.SetString("SavedPassword", Password);
                            }
                            else
                            {
                                Google2uGUIUtil.SetString("SavedUsername", string.Empty);
                                Google2uGUIUtil.SetString("SavedPassword", string.Empty);
                            }
                        }
                        else if (!_SaveCredentials)
                        {
                            Google2uGUIUtil.SetString("SavedUsername", string.Empty);
                            Google2uGUIUtil.SetString("SavedPassword", string.Empty);
                        }
                        ...
                        EditorGUILayoutEx.ToggleInput(
                            LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_AUTO_LOGIN), ref _AutoLogin,
                            "AutoLogin");

                        var content = new GUIContent(MyGUILayout.LoginButton, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_LOGIN));
                    ...
                    else
                    {
                        ...
                        var content = new GUIContent(MyGUILayout.LogoutButton, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_LOGOUT));
                        if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                        {
                            if (!InstanceData.Commands.Contains(GFCommand.DoLogout))
                                InstanceData.Commands.Add(GFCommand.DoLogout);
                        }
                        EditorGUILayout.LabelField(LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_LOGGED_IN_AS) + ": " + Username);
        ...
        void DrawWorkbooksGUI()
        {
            ...
            if (workbooksFadeArea.Show())
            {
                    ...
                    if (showWorkbooksAccount.Show())
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Google2uWorkbook.DrawGUIList(MyGUILayout);
                                }
                            }
                        }
                    }
                ...
                if (showWorkbooksManual.Show())
                {
                    ...
                    {
                        var deleteWorkbooks = InstanceData.ManualWorkbooksDisplay.Where(in_Google2uSpreadsheet => in_Google2uSpreadsheet.DrawGUIList(MyGUILayout) == false).ToList();
