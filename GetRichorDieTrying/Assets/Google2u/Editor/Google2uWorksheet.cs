...
namespace Google2u
{
    ...
    public class Google2uRow
    {
        ...
        {
            get { return Cells[in_i]; }
            set { Cells[in_i] = value; }
        }
    ...
    public class Google2uWorksheet
    {
        ...
        public void Update(bool in_isActive)
        {
        ...
        public void DoDataValidation(object in_worksheet)
        {
            var worksheet = in_worksheet as Google2uWorksheet;
            if (worksheet == null)
                return;
        ...
        public void DrawGUIFull(EditorGUILayoutEx in_layout)
        {
            ...
            else
            {
                ...
                if (ActiveCell != null)
                {
                    if (!string.IsNullOrEmpty(ActiveCell.Tooltip))
                    {
                ...
                for (var i = 0; i < RowsDisplay.Length; i++)
                {
                    ...
                    if (i == 1 && UseTypeRow)
                    {
                        for (var j = 0; j < RowsDisplay[i].Count; j++)
                        {
                            ...
                            //GUI.Button(curRect, Convert.ToString(cellType), in_layout.CellTypeButton);
                            //var newCellType = (SupportedType)EditorGUILayout.EnumPopup(cellType, in_layout.CellTypeButton, GUILayout.Width(ColOptions[j].Width), GUILayout.Height(24));

                            //if (newCellType != cellType)
                            //{
                            //    myCell.MyType = newCellType;
                            //    myCell.SetValueFromType();
                            //    UpdateCellTypes = true;
                            //    UpdateValidation = true;
                            //}
