...
public class Hud : MonoBehaviour
{
    ...
    private Google2u.SampleLocalization _Localization;
	...
	void Start ()
	{
	    ...
	    _Localization = Google2u.SampleLocalization.Instance;
	}
    ...
    void OnGUI()
    {
        ...
        if (GUI.Button(_SpawnButtonRect, _Localization.GetRow(SampleLocalization.rowIds.STR_SPAWN).GetStringData(Language)))
        {
            if (OnSpawn != null)
                OnSpawn();
        }

        if (GUI.Button(_EnglishButtonRect, _Localization.GetRow(SampleLocalization.rowIds.STR_ENGLISH).GetStringData(Language)))
        {
            Language = "en";
        }
        if (GUI.Button(_SpanishButtonRect, _Localization.GetRow(SampleLocalization.rowIds.STR_SPANISH).GetStringData(Language)))
        {
            Language = "es";
        }
        if (GUI.Button(_FrenchButtonRect, _Localization.GetRow(SampleLocalization.rowIds.STR_FRENCH).GetStringData(Language)))
        {
            Language = "fr";
        }
        ...
        if (CurAiScript != null)
        {
            var name =   _Localization.GetRow(SampleLocalization.rowIds.STR_NAME).GetStringData(Language) + ": " + CurAiScript.BotName;
            var level =  _Localization.GetRow(SampleLocalization.rowIds.STR_LEVEL).GetStringData(Language) + ": " + CurAiScript.BotLevel;
            var hp =     _Localization.GetRow(SampleLocalization.rowIds.STR_HEALTH).GetStringData(Language) + ": " + CurAiScript.Health;
            var weapon = _Localization.GetRow(SampleLocalization.rowIds.STR_WEAPON).GetStringData(Language) + ": " + _Localization.GetRow(CurAiScript.WeaponID).GetStringData(Language);
