...
- (void)createWebView
{
	...
	// Allow webview to be transparent
...
{
	// Cache normalised frame
	self.normalisedFrame	= ConvertToNormalisedRect(frame);
	
	// Convert to screen space
	[super setFrame:frame];
	
    // Update subview components frame
    [self updateSubviewFrame];
}
...
- (void)updateSubviewFrame
{
	...
	else if ([self controlType] == WebviewControlTypeCloseButton)
	{
		...
		CGSize webviewSize	= CGSizeMake(viewSize.width - closeButtonSize.width, viewSize.height - closeButtonSize.height);
		
		// Set webview origin and size
		webviewFrame.origin	= CGPointMake(viewSize.width * 0.5f - webviewSize.width * 0.5f, viewSize.height * 0.5f - webviewSize.height * 0.5f);
...
- (void)show
{
	...
	// Set frame
	[self setFrame:ConvertToScreenSpace(self.normalisedFrame)];
}
