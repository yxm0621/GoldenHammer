...
- (void)onReadPermissionGranted:(ABAddressBookRef)addressBook
{
	...
	CFArrayRef allPeople            = ABAddressBookCopyArrayOfAllPeopleInSourceWithSortOrdering(addressBook,
																								source,
kABPersonSortByFirstName);
	CFIndex totalContacts           = ABAddressBookGetPersonCount(addressBook);
	...
	for (int iter = 0; iter < totalContacts; iter++)
	{
		...
		NSString *firstName             = (NSString *)firstNameRef;

		if (firstName == NULL)
		{
			eachContactData[kFirstName]	= kNSStringDefault;
		}
		else
		{
		...
		NSString *lastName              = (NSString *)lastNameRef;
		
		if (lastName == NULL)
		{
			eachContactData[kLastName]	= kNSStringDefault;
		}
		else
		{
