...
bool canSendMail ()
{
    return [[SharingHandler MailShare] canSendMail];
}
...
{
	[[SharingHandler MailShare] sendMailWithSubject:ConvertToNSString(subject)
...
{
    return [[SharingHandler MessagingShare] isMessagingAvailable];
}

void sendTextMessage (const char* body, const char* recipients)
{
    [[SharingHandler MessagingShare] sendMessageWithBody:ConvertToNSString(body)
...
{
	BOOL _canShareText	= [[SharingHandler WhatsAppShare] canShareTextMessage];
	BOOL _canShareImage	= [[SharingHandler WhatsAppShare] canShareImage];
...
{
	[[SharingHandler WhatsAppShare] shareTextMessage:ConvertToNSString(message)
...
{
	[[SharingHandler WhatsAppShare] shareImage:[Utility CreateImageFromByteArray:imageByteArray ofLength:byteArrayLength]
									completion:^(bool completed) {
										// Notify unity
										NSString *completedStr	= [NSString stringWithFormat:@"%d", completed];
										NotifyEventListener(kWhatsAppShareFinished, [completedStr UTF8String]);
...
{
    [[SharingHandler Instance] shareMessage:ConvertToNSString(message)
